...
- Deployment config is split into `docker/test-values.yaml` and `docker/prod-values.yaml`.
...
- `DJANGO_SETTINGS_MODULE=dawgpath_pipeline_admin.tests.test_settings ./venv/bin/python -m unittest discover -s dawgpath_pipeline_admin/tests` -> 3 tests OK.
...
- `docker/test-values.yaml`
- `docker/prod-values.yaml`
